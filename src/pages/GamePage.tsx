...
export default function GamePage() {
    ...
    const loadRoom = useCallback(async () => {
        ...
        try {
            ...
            // ★ playerId 결정: sessionStorage 우선, 없으면 nickname으로 players에서 탐색
            let pid = Number(sessionStorage.getItem('bingo_playerId')) || null;

            if (!pid && myNickname) {
                const me = r.players.find((p) => p.nickname === myNickname);
                if (me) {
                    pid = me.playerId;
                    sessionStorage.setItem('bingo_playerId', String(pid));
                }
            }
    ...
    const onGameEvent = useCallback(async (event: GameEvent) => {
        const {
            type, calledWord: cw, callerNickname,
            playerStates, winnerNickname, message,
            revealedBoards, nextTurnNickname,
        } = event;
        ...
        if (type === 'WORD_CALLED' || type === 'BINGO_ACHIEVED') {
            if (playerStates) {
                setPlayers((prev) => prev.map((p) => {
                    const u = playerStates.find((s) => s.playerId === p.playerId);
                    return u ? { ...p, bingoCount: u.bingoCount } : p;
                }));
            }
            // ★ 항상 최신 pid로 보드 조회
            const pid = Number(sessionStorage.getItem('bingo_playerId')) || myPlayerId;
            if (pid && pid > 0) await loadBoard(pid);
    ...
    const onPlayersEvent = useCallback((event: GameEvent) => {
        if (event.playerStates) {
            setPlayers((prev) => prev.map((p) => {
                const u = event.playerStates?.find((s) => s.playerId === p.playerId);
                return u ? { ...p, bingoCount: u.bingoCount } : p;
            }));
        }
    ...
    return (
        <div style={{ minHeight: '100vh', padding: '20px 16px' }}>
            {toast && (
                <div className="toast" style={{ borderColor: 'var(--accent-gold)', fontWeight: 700, fontSize: '1rem' }}>
                    {toast}
                </div>
            )}
            ...
            <div style={{
                ...
                display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 290px', gap: 20,
            }}>
                {/* ── 왼쪽: 보드 ── */}
                <div>
                    {/* 턴 배너 */}
                        ...
                        <div style={{
                            ...
                            {isMyTurn ? (
                                <>
                                    ...
                                    <span style={{ fontWeight: 700, color: 'var(--primary)' }}>내 차례입니다! 단어를 선택하세요</span>
                                </>
                            ) : (
                                <>
                    ...
                    <div className="card-elevated" style={{ padding: 16, marginBottom: 14 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                            <span style={{ fontSize: '0.85rem', color: 'var(--text-2)', fontWeight: 600 }}>내 빙고판</span>
                            <div style={{ display: 'flex', gap: 5 }}>
                                {Array.from({ length: 5 }).map((_, i) => {
                                    const achieved = i < (myBoard?.bingoCount ?? 0);
                                    return (
                                        <div key={i} style={{
                                            width: 26, height: 26, borderRadius: 6, fontSize: '0.7rem',
                                            display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900,
                                            background: achieved ? 'linear-gradient(135deg, var(--accent-gold), #ff9f1c)' : 'var(--bg)',
                        ...
                        {isMyTurn && !gameOver && (
                            <p style={{ color: 'var(--primary)', fontSize: '0.8rem', marginTop: 10, textAlign: 'center', fontWeight: 600 }}>
                                👆 단어를 클릭해서 선택하세요
                            </p>
                        )}
                    </div>
                    ...
                    {calledWord && (
                        <div style={{
                            textAlign: 'center', padding: '12px 16px', background: 'var(--bg-card)',
                            ...
                            <span style={{ color: 'var(--accent-gold)', fontWeight: 700, fontSize: '1.1rem' }}>"{calledWord}"</span>
                        </div>
                    )}

                    {/* 게임 로그 */}
                    <div className="card" style={{ padding: 16 }}>
                        <p style={{ color: 'var(--text-2)', fontSize: '0.78rem', fontWeight: 600, marginBottom: 10, letterSpacing: '0.05em' }}>
                            게임 로그
                        </p>
                        <div ref={logRef} style={{ maxHeight: 160, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 4 }}>
                            {log.length === 0 ? (
                                <p style={{ color: 'var(--text-2)', fontSize: '0.82rem', textAlign: 'center', padding: '20px 0' }}>
                                    게임이 시작되었습니다!
                                </p>
                            ) : (
                                log.map((entry, i) => (
                ...
                <div>
                    <div className="card" style={{ padding: 16, position: 'sticky', top: 20 }}>
                        ...
                        <PlayerList players={players} currentTurnNickname={currentTurn?.nickname} myNickname={myNickname} />
                    </div>
                </div>
            </div>
            ...
            {gameOver && (
                <div style={{
                    ...
                    <div className="card-elevated animate-fadeInUp" style={{
                        width: '100%', maxWidth: 720, padding: 36, maxHeight: '90vh', overflowY: 'auto',
                    }}>
                        <div style={{ textAlign: 'center', marginBottom: 32 }}>
                            <div style={{ fontSize: 64, marginBottom: 12 }}>🏆</div>
                            <h1 style={{ fontSize: '2rem', color: 'var(--accent-gold)', marginBottom: 8 }}>게임 종료!</h1>
                            <p style={{ fontSize: '1.1rem', color: 'var(--text-2)' }}>
                <span style={{ color: 'var(--accent-gold)', fontWeight: 700, fontSize: '1.4rem' }}>
                  {gameOver.winnerNickname}
                </span>님이 5빙고로 우승하였습니다!
                            </p>
                        </div>

                        {gameOver.revealedBoards.length > 0 && (
                            <div>
                                <p style={{ color: 'var(--text-2)', fontSize: '0.82rem', fontWeight: 600, letterSpacing: '0.05em', marginBottom: 16 }}>
                                    최종 빙고판 공개
                                </p>
                                    ...
                                    {gameOver.revealedBoards.map((board) => (
                                        <div key={board.playerId} className="card" style={{ padding: 16 }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
                                                ...
                                                {board.isWinner && <span className="badge badge-gold">🏆 우승</span>}
                                                <span className="badge badge-primary">{board.bingoCount}빙고</span>
                                            </div>
                                            <BingoBoard words={board.words} checkedCells={board.checkedCells} isMyTurn={false} disabled />
                                        </div>
                                    ))}
                                </div>
                            </div>
